...
import { SET_MY_LOCATIONS, ADD_MY_LOCATION, LOCATION_FETCHED, LOCATION_DELETED } from '../types';
...
export default function shootinglocations(state = [], action = {}) {
  switch(action.type) {
    ...
    case LOCATION_FETCHED:
      const index = state.findIndex(location => location.id === action.location.id);
      if (index > -1) {
